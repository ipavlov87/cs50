...
int main(int argc, string argv[])   //Cipher to encode messages using a string of letters, attempt #3, submitted to CS50, simplified from #2
{
    ...
    for (int o = 0, p = 0; o < strlen(plaintext); o++)  //starts the cipher at position 0 in the plaintext
    {
        ...
        {
            if (islower(plaintext[o]))
            {
                q = 'a';
            }
            else if (isupper(plaintext[o]))
            {
                q = 'A';
            }
